...
async function endS(api, event, getText, Users, Currencies, object) {
	...
	for (let i = 0; i < object.players - 1; i++) {
		...
		result = (botRank > rank) ? `Thua (-${playerD.bet}$)` : `Thắng (+${playerD.bet + "$ x" + bonus})`;
		if (botRank == rank) {
			result = (playerPoints == getBotPoint || rank >= 4) ? "Hòa" : (rank == 1) ? (playerPoints < getBotPoint) ? `Thắng (+${playerD.bet}$)` : `Thua (-${playerD.bet}$)` : (rank == 2) ? (playerPoints > getBotPoint) ? `Thắng (+${playerD.bet}$)` : `Thua (-${playerD.bet}$)` : (rank == 3) ? (playerPoints < getBotPoint) ? `Thắng (+${playerD.bet + " x" + object.bonus.superWinBonus}$)` : `Thua (-${playerD.bet}$)` : '';
		}
		if (result == "Hòa") await increaseMoney(playerD.id, playerD.bet);
