...
module.exports.run = async function ({ api, event, args, Users, Currencies }) {
    ...
    switch (args[0]) {
        ...
        case '-s': {
            if (!existsSync(pathData)) {
                return api.sendMessage({body: "Bạn chưa đăng kí game!", attachment: await this.image('https://i.imgur.com/6NYfksi.gif')}, threadID, messageID);
            }
        ...
        case 'xd': {
            if (!existsSync(pathData)) {
                return api.sendMessage({body: "Bạn chưa đăng kí game!", attachment: await this.image('https://i.imgur.com/zn0ifgY.gif')}, threadID, messageID);
            }
        ...
        default: {
            return api.sendMessage({body: `=====[ COINMASTER ]=====\n/coinmaster -dk: Đăng kí\n/coinmaster -s: Vòng quay game\n/coinmaster (build/xd): Xây dựng đảo\n/coinmaster shop: Quy đổi tiền - coins, mua spin\n/coinmaster (me/info): Xem thông tin về bạn\n/coinmaster top: Xem top level trên server\n\n`, attachment: await this.image('https://i.imgur.com/rQPzm8J.jpeg')}, threadID, messageID);
        }
    }
}
