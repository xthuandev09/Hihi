...
module.exports.run = async function ({ api,event,args }) {
    var axios = require('axios');
        try {
            if (args.length != 2 || !args[0] || !args[1]) return api.sendMessage("Hãy Bấm: emojimix 😢 🤣 hoặc emojimix emoji1 emoji2\nNếu không được hãy đổi vị trí 2 icon",event.threadID,event.messageID);
                var emoji1 = "u"+emojiUnicode(args[0]),emoji2 = "u"+emojiUnicode(args[1]);
                    var { data } = await axios.get(`https://hoanghao.me/api/emojimix?emoji/${emoji1}/${emoji1}_${emoji2}.png`,{  method: 'GET',  responseType: 'arraybuffer' });
