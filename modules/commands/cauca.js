...
module.exports.run = async function ({ event, api, args }) {
    ...
    switch ((args[0] || "").toLowerCase()) {
        ...
        case "-r": {
            try {
                if (existsSync(dirUser)) return api.sendMessage("[🌸 Fishing 🌸] Bạn đã từng đăng ký câu cá tại khu vực này!\nSử dụng !cauca register để đăng kí", threadID, messageID);
                var newData = {};
                newData.fishingrod = {
                    "rodType": 0,
                    "enchantRod": {}
        ...
        case "shop": {
            if (!existsSync(dirUser)) return api.sendMessage("[🌸 Fishing Shop 🌸] Bạn chưa đăng ký câu cá để có thể sử dụng chức năng mua đồ!\nSử dụng !cauca register để đăng kí", threadID, messageID);
        ...
        default: {
            try {
                if (!existsSync(dirUser)) return api.sendMessage("[🌸 Fishing 🌸] Bạn chưa đăng ký câu cá tại khu vực này!\nSử dụng !cauca register để đăng kí", threadID, messageID);
                var dataUser = JSON.parse(readFileSync(dirUser, "utf-8"));
