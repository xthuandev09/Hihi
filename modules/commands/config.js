...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      ...
      return reply(`Danh sách tin nhắn chưa đ��c của bot:\n\n${msg}`);
    }
    ...
    if (!body) return reply("Vui lòng nhập uid của những người bạn muốn bỏ chặn trên messenger, có thể nh���p nhiều id cách nhau bằng dấu cách hoặc xuống dòng", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
