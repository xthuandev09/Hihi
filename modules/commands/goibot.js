...
module.exports.handleEvent = function({ api, event }) {
  ...
  if ((event.body.toLowerCase() == "Bán Bot") || (event.body.toLowerCase() == "ban bot")) {
    return api.sendMessage("Đừng bán mà cho bot xin lỗi vì đã chê admin ngu và nhìu từ khác nhưng bot yêu admin lắm🥰", threadID);
  };
  
  if ((event.body.toLowerCase() == "Huy Bao") || (event.body.toLowerCase() == "huy bao")) {
    return api.sendMessage(" Huy bảo là 1 thg culi múp s1tg của qthai", threadID);
  };
  
  if ((event.body.toLowerCase() == "trần Vũ") || (event.body.toLowerCase() == "tran vu")) {
    return api.sendMessage("Tran Vu Là 1 Con Chó Treo Ko Tới 1 Tiếng Rớt", threadID);
  };

  if ((event.body.toLowerCase() == "Quoc Thai") || (event.body.toLowerCase() == "quoc thai")) {
    return api.sendMessage(" Là 1 thg bá treo chuyên giet lũ đú ảo ửa", threadID);
  };
