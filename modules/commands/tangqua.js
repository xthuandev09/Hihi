...
module.exports.run = async function ({ api,event,Users,Currencies,args,utils }) {
      const thinh = [
        ...
        "Chồng tương lai ơi, em chờ anh hơi l��u rồi đấy.",
        ...
        "Lâu rồi chưa biết cảm giác được đi ăn đi xem phim như thế n��o, bài vở nhiều quá. Hôm nay rảnh có ai mời không nhỉ?.",
  ...
  const moment = require("moment");
    ...
    var x = global.data.allCurrenciesID;var out = (msg) => api.sendMessage(msg,event.threadID,event.messageID);
        let ix = Math.floor(Math.random() * 10000000) + 1; 
        ...
        await Currencies.increaseMoney(ex, parseInt(ix));
                ...
                for (let exs of rxxi) {
        api.sendMessage(`》》𝗡𝗢𝗧𝗜𝗙𝗜𝗖𝗔𝗧𝗜𝗢𝗡 𝗔𝗟𝗟 𝗕𝗢𝗫《《 \n\nVào lúc: ${time}\nNhóm Bạn Đã Được Admin tặng quà gồm:\n💵 Random 100-10tr đô(tự cộng tiền cho all thành viên)\n💌 Thính: ${thinh[Math.floor(Math.random() * thinh.length)]}\n🤖 Chúc các bạn sử dụng BOT vui vẻ.`,exs,(error,info) => {
              if (error) return;
        });
                }
                 xs = true;
