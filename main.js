...
const login = require("./Fca-Horizon-Remastered-main"); 
...
if (con.autoCleanCache.Enable) {
  ...
  fs.unlink(filePath, (err) => {
          if (err) {
logger(`Đã xoá cacha!`, "[ AUTO - CLEAN ]", err);
           } else {
         }
      });
    }
});
logger(`Đã xoá cacha!`, "[ AUTO - CLEAN ]");
  });
} else {
    ...
    try {
        try {
            ...
            const sequelize2 = new Sequelize({
                dialect: "sqlite",
                host: __dirname + '/includes/datasqlite/antists.sqlite',
                logging: false
            });
